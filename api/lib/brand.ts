...
export const brandRenderColors = {
  light: {
    background: brandTokens.color.hex.light.base.background,
    text: brandTokens.color.hex.light.base.text,
    primary: brandTokens.color.hex.light.base.primary,
    accent: brandTokens.color.hex.light.base.accent,
  },
  dark: {
    background: brandTokens.color.hex.dark.base.background,
    text: brandTokens.color.hex.dark.base.text,
    primary: brandTokens.color.hex.dark.base.primary,
    accent: brandTokens.color.hex.dark.base.accent,
  },
} as const;

const generateCssVariables = (scheme: "light" | "dark") => {
  const colors =
    scheme === "light" ? brandTokens.color.light : brandTokens.color.dark;
  const bgScale =
    scheme === "light"
      ? brandTokens.color.light.background
      : brandTokens.color.dark.background;
  const textScale =
    scheme === "light"
      ? brandTokens.color.light.text
      : brandTokens.color.dark.text;
  const primaryScale =
    scheme === "light"
      ? brandTokens.color.light.primary
      : brandTokens.color.dark.primary;
