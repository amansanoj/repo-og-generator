...
export function resolvePublicDir(): string {
  ...
  const publicDir = candidates.find((dir) =>
    existsSync(join(dir, appConfig.files.fonts.regular.file)),
  );
  if (!publicDir) {
...
function applyBrandColors(svg: string): string {
  return svg
    ...
    .replaceAll(brandSvgPlaceholders.accent, brandRenderColors.light.accent)
    .replaceAll(
      brandSvgPlaceholders.surface,
      brandRenderColors.light.background,
    );
}
...
export function loadOgAssets(publicDir: string): OgAssets {
  ...
  return {
    regularFont: readFileSync(
      join(publicDir, appConfig.files.fonts.regular.file),
    ).buffer,
    boldFont: readFileSync(join(publicDir, appConfig.files.fonts.bold.file))
      .buffer,
    svgDataUrls,
  };
}
...
export function loadFaviconSvg(publicDir: string): string {
  return applyBrandColors(
    readFileSync(join(publicDir, appConfig.files.favicon), "utf8"),
  );
}
